...
const QR = {
  ...
  toSvg(qr, opts = {}) {
    ...
    const moduleSize = +opts.moduleSize || 10;
    let margin = opts.margin != null ? +opts.margin : 4; if (!Number.isFinite(margin) || margin < 0) margin = 4;
    const fg = escAttr(opts.fg || '#000'), bg = escAttr(opts.bg || '#fff');
